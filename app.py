...
# ==============================
# 🔄 GitHub 热更新引擎 (前后端双轨同步)
# ==============================
@app.route('/api/update', methods=['POST'])
def api_update():
    try:
        # ---- 1. 拉取前端文件 index.html ----
        github_html_url = "https://raw.githubusercontent.com/kgdjcf624/Matrix/main/templates/index.html"
        req_html = urllib.request.Request(github_html_url, headers={'User-Agent': 'Mozilla/5.0'})
        try:
            with urllib.request.urlopen(req_html) as response:
                html_content = response.read().decode('utf-8')
        except Exception:
            # 尝试 fallback 到 master 分支
            github_html_url_master = "https://raw.githubusercontent.com/kgdjcf624/Matrix/master/templates/index.html"
            req_html = urllib.request.Request(github_html_url_master, headers={'User-Agent': 'Mozilla/5.0'})
            with urllib.request.urlopen(req_html) as response:
                html_content = response.read().decode('utf-8')
                
        # ---- 2. 拉取后端文件 app.py ----
        github_app_url = "https://raw.githubusercontent.com/kgdjcf624/Matrix/main/app.py"
        req_app = urllib.request.Request(github_app_url, headers={'User-Agent': 'Mozilla/5.0'})
        try:
            with urllib.request.urlopen(req_app) as response:
                app_content = response.read().decode('utf-8')
        except Exception:
            # 尝试 fallback 到 master 分支
            github_app_url_master = "https://raw.githubusercontent.com/kgdjcf624/Matrix/master/app.py"
            req_app = urllib.request.Request(github_app_url_master, headers={'User-Agent': 'Mozilla/5.0'})
            with urllib.request.urlopen(req_app) as response:
                app_content = response.read().decode('utf-8')

        # 校验拉取内容的有效性
        if not html_content or "<html" not in html_content:
            return jsonify({'error': '拉取到的前端内容无效，更新被中止'}), 400
            
        if not app_content or "Flask" not in app_content:
            return jsonify({'error': '拉取到的后端内容无效，更新被中止'}), 400

        # ---- 执行覆盖写入 ----
        # 1. 覆盖前端 (确保 templates 文件夹存在)
        template_path = os.path.join(app.template_folder, 'index.html')
        with open(template_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
            
        # 2. 覆盖后端 (定位当前正在运行的 app.py 绝对路径)
        # 注意：如果被 pyinstaller 打包为 exe，这段代码无法覆盖运行中的可执行文件，仅针对源码环境生效。
        app_path = os.path.abspath(__file__)
        with open(app_path, 'w', encoding='utf-8') as f:
            f.write(app_content)
            
        return jsonify({'message': '前后端同步更新成功！页面即将刷新，后端请在终端重启(Ctrl+C后重新运行)以应用新版本。'})
    except Exception as e:
        print("❌ 更新失败:", str(e))
        return jsonify({'error': str(e)}), 500
